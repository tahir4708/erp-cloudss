...
def main() -> int:
    ...
    parser.add_argument(
        "--mode",
        default="indicators",
        choices=["indicators", "candles"],
        help="Analysis mode: indicators (EMA/RSI/MACD) or candles (price action only)",
    )
    ...
    print("=" * 48)
    print(f"  {data['symbol']}  ·  {data['timeframe']}  ·  {data['analysis_mode']}")
    print("=" * 48)
    print(f"  1) Side          : {data['side']}")
    print(f"  2) Entry point   : {data['entry']}")
    print(f"  3) Lot size      : {data['lot_size']}")
    print(f"  4) TP / SL       : {data['take_profit']}  /  {data['stop_loss']}")
    print(f"     Distances    : TP ${data['tp_distance']}  |  SL ${data['sl_distance']}  (price $)")
    print(f"  5) Win %         : {data['win_probability']}%  (confidence {data['confidence']}%)")
    print("-" * 48)
